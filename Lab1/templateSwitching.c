...
int main(int argc, char *argv[]) {
   ...
   double pAllNotBusy = pow(pPSusersBusy, nPSusers - 1);
   double pOneBusy = pPSusers * pAllNotBusy;
   double pExactlyOne = nPSusers * pOneBusy;
   double pTenBusy = pow(pPSusers, 10) * pow(pPSusersNotBusy, nPSusers);
   ...
   {
       pTenMoreBusy += pow(pPSusers, i) * pow(pPSusersNotBusy, nPSusers) * factorial(nPSusers) / (factorial(i) * factorial(nPSusers - i));
   }
